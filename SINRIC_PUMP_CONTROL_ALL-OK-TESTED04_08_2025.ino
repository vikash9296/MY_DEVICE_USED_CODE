...
// WiFi credentials
#define WIFI_SSID     "Sanjaykumar"
#define WIFI_PASS     "Hello@321"
...
void pulseRelay(int relayPin) {
  // Block relay activation in first 3 seconds after boot
...
void setupSinricPro() {
  ...
  SinricPro.restoreDeviceStates(false);  // Don't restore last ON state
...
void setup() {
  Serial.begin(115200);
  bootTime = millis();  // For blocking early relay triggers
  ...
  setupSinricPro();  // Start Sinric Pro after WiFi connected
}
