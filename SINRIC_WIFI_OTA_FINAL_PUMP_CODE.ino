/******** Sinric Pro Momentary Relay Control + WiFiManager + Auto Reconnect ********/
...
void pulseRelay(int pin) {
  if (millis() - bootTime < 3000) return;  // ignore pulse during boot
...
void setup() {
  ...
  WiFiManager wm;
  bool res = wm.autoConnect("Pump-Setup", "12345678");  // AP name, password
  if (!res) {
    Serial.println("Failed to connect.");
    ESP.restart();
  }
  Serial.println("WiFi connected!");
  WiFi.setAutoReconnect(true);  // ✅ Important for auto reconnect
