...
BLYNK_APP_CONNECTED() {
  ...
  digitalWrite(wifiLed, HIGH); // Turn on LED when app is connected
}

// This function is called every time the Blynk App disconnects from the device
BLYNK_APP_DISCONNECTED() {
  Serial.println("Blynk App disconnected.");
  // You might want to keep the LED on if the device is still connected to the cloud,
  // or turn it off if you want to indicate app disconnection specifically.
  // For now, let's keep it on if cloud is connected, or off if not.
  if (!Blynk.connected()) {
    digitalWrite(wifiLed, LOW); // Turn off LED if not connected to Blynk Cloud
  }
}
